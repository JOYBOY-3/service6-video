...
var (
	...
	// Cancelled indicates the operateion was canceled (typpically by the caller)
	Canceled = ErrCode{value: 1}

	// Unknown error. An example of where this erro may be retuned is
	// if a Status value recieved from another address space belongs to
	// an error-space that is not known in this adress space. Also
	// errors raised by APIs that fo not return enough error information
...
// UnmarshalText implement the unmarsha; interface for JSON conversions.
